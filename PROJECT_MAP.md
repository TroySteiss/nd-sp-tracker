...
| property | viewProperty | per-property: financial summary, projects by phase, GL reconciliation, update email. **On hold + Notes render in their own "Future Projects" panel below the Projects panel (2026-09-08)** — amber-accented `.future-panel`, collapsible header, own hide chips; the main panel's groups are active/paid/discussed/done (+Shared/ATL) with **Completed hidden by default** (`PFILT.hide.done`). **Reconciliation & flags is collapsed by default** (`PFILT.reconOpen`; header chips still show "N untied"/"N to review") and carries a "Not tied to a project" total whose *Review in ledger ↓* button points the GL filters at exactly those lines. **GL ledger filters**: text search (commits on Enter — a per-keystroke re-render drops focus), a "Tie out: pick a project" select that shows only that project's linked lines (ignores the under-$1K switch) plus a linked-vs-tracker-cost banner, a shown-lines total chip and a TOTAL table row. Project-row tracker: **yellow = first UNTICKED step** ("up next"), all done steps green; hold/note rows get no yellow |
