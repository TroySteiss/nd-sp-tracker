...
/* Leads the name ("⚠ Lender Repairs") rather than trailing it: name cells
   ellipsize, and a trailing symbol is the first thing clipped away. */
const lenderSym=p=>(lenderFlagged(p)||lenderNamed(p))
  ? el('span',{title:'Lender-required item'+(lenderFlagged(p)?' — '+String(p.lenderFlag).trim():''),
      style:'color:#c02a1e;font-weight:800;margin-right:5px'},'⚠︎')
  : null;
...
/* ---------- Long-range plan (mirrors shared/domain.ts, migration 028) ----------
...
function planHorizonEnd(code){
  ...
  const prop=PROP(code)||{}; const snap=S.cash[code]||{};
  let end=null;
  const o=Number(prop.planEndYear);
  if(o>=2000&&o<=2099)end=o;
  if(end==null&&snap.loanDue){ const m=String(snap.loanDue).match(/(\d{4})/); if(m)end=+m[1]; }
  if(end==null)end=nowYear+4;
  return Math.min(Math.max(end,nowYear),nowYear+14);
}
/* Year columns for a property's plan grid: now..end plus stray data years. */
...
const phaseChip=p=>{ if(!isPhaseP(p))return null; const sibs=phasesOf(p.phaseGroup);
  return el('button',{class:'chip phase',title:'Phased program: '+(p.phaseOf||'')+' — phase '+(p.phaseSeq||'?')+' of '+sibs.length+'. Click to edit all phases in one table.',
    onclick:e=>{e.stopPropagation();openProgramEditor(p.phaseGroup);}},
...
function rail(){
  ...
  nav.append(item('plan','🗓','Long-Range Plan',S.projects.filter(onPlan).length||null));
  if(IS_ADMIN){
...
function mainCol(){
  ...
  const views={dashboard:viewDashboard,projects:viewProjects,inhouse:viewInHouse,contracts:viewContracts,property:viewProperty,cash:viewCash,plan:viewPlan,data:viewData,directory:viewDirectory,changelog:viewChangelog,settings:viewSettings};
...
function viewDashboard(){
  ...
  const calcProjStats=pr=>{
    ...
    for(let q=currentQtr+1;q<=4;q++){ const ov=distQtrs['retQ'+q], br=budgetByQ[q]; const rate=(ov!=null&&!isNaN(Number(ov)))?Number(ov):(br!=null&&!isNaN(Number(br)))?Number(br):madePct; futureAccretion+=((rate-effectiveSentPct)/100/4)*capitalDollars; }
    ...
    // Base = Cash After Distribution (cushion Col V); fall back to cash today only when the report lacks it.
    const cashAfterDist=c.cashAfterDist!=null?Number(c.cashAfterDist):null;
    const projBase=cashAfterDist!=null?cashAfterDist:cashToday;
    const projEndCash=projBase+projInterest-projRemainingSpend+futureAccretion;
    const cpd=pr.units?projEndCash/Number(pr.units):null;
    const budget=Number(pr.spBudget)||0;
    const yyyyBudgetProj=budget-spent-projRemainingSpend;
    const fy=(/^\d{4}/.test(asOf)?String(asOf).slice(0,4):today().slice(0,4));
    return {spent,cashToday,cashAfterDist,projRemainingSpend,projInterest,futureAccretion,projEndCash,cpd,budget,yyyyBudgetProj,fy,
            units:Number(pr.units)||0,madePct,effectiveSentPct,futureQtrs};
  };
  ...
  const propRowEl=(pr,s)=>el('div',{class:'psum-row',style:'display:flex;align-items:stretch;border-bottom:1px solid var(--line-2)'},
    ...
    statCells(s,s.fy,`${s.futureQtrs}q returns`));
...
function discussedPanel(list){
  ...
  view.slice(0,80).forEach(p2=>{
    ...
    r.append(propChip(p2.property),
      el('div',{style:'flex:1;min-width:0'}, el('div',{style:'font-weight:600;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'},lenderSym(p2),p2.name,projSym(p2)),
...
function projectCard(p){
  ...
  c.append(top);
  c.append(el('div',{class:'nm'},lenderSym(p),p.name,projSym(p),phaseChip(p),qtyChip(p)));
...
function projectsTable(list){
  ...
  list.forEach(p=>{
    ...
    tb.append(el('tr',{class:'clickrow',onclick:()=>openProject(p.id)},
      ...
      td(el('div',{style:'font-weight:600;max-width:280px'},lenderSym(p),p.name,projSym(p),
...
/* =========================================================
   PROGRAM EDITOR — bulk-edit every phase (name / contractor / cost /
   dates) in one table, no per-phase editor round-trips. Explicit Save
   PATCHes only the phases that actually changed.
========================================================= */
function openProgramEditor(group){
  const sibs=phasesOf(group);
  if(!sibs.length){ toast('No phases found for that program.'); return; }
  const scrim=el('div',{class:'scrim modal-center',onclick:e=>{if(e.target===scrim)scrim.remove();}});
  // .modal-center .sheet is 560px by default — this is a six-column editing
  // table, so widen it inline (inline width beats the class rule).
  const sheet=el('div',{class:'sheet',style:'width:min(920px,96vw)'});
  const err=el('div',{style:'color:var(--rust);font-size:12px;min-height:16px'});
  const drafts=sibs.map(sb=>({id:sb.id,name:sb.name||'',contractor:sb.contractor||'',
    cost:sb.anticipatedCost==null?'':String(sb.anticipatedCost),start:sb.plannedStart||'',end:sb.plannedEnd||'',inHouse:!!sb.inHouse}));
  const orig=JSON.parse(JSON.stringify(drafts));
  const cellInp=(d,key,attrs={})=>el('input',{value:d[key],style:'width:100%;min-width:0;padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:12.5px',
    oninput:e=>{d[key]=e.target.value;},...attrs});
  const t=el('table',{class:'tbl'});
  t.append(el('thead',{},tr(th('#'),th('Phase name'),th('Contractor'),th('Est. cost ($)','r'),th('Start'),th('End'),th(''))));
  const tb=el('tbody');
  sibs.forEach((sb,i)=>{
    const d=drafts[i];
    const pm=phaseMeta(phase(sb))||{};
    tb.append(tr(
      td(el('span',{class:'mono'},String(sb.phaseSeq||i+1))),
      el('td',{style:'min-width:220px'},cellInp(d,'name')),
      el('td',{style:'min-width:130px'},cellInp(d,'contractor',{placeholder:'—'})),
      td(cellInp(d,'cost',{type:'number',min:'0',step:'any',style:'width:110px;text-align:right;padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:12.5px',
        ...(d.inHouse?{disabled:true,title:'In-house phase — cost is tracked as progress in its own editor'}:{})})),
      td(cellInp(d,'start',{type:'date',style:'padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:12px'})),
      td(cellInp(d,'end',{type:'date',style:'padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:12px'})),
      td(el('span',{class:'chip '+(pm.chip||'')},pm.label||''))));
  });
  t.append(tb);
  const saveBtn=el('button',{class:'btn accent',onclick:async()=>{
    const changed=drafts.filter((d,i)=>JSON.stringify(d)!==JSON.stringify(orig[i]));
    if(!changed.length){ scrim.remove(); return; }
    if(changed.some(d=>!d.name.trim())){ err.textContent='Every phase needs a name.'; return; }
    saveBtn.disabled=true; saveBtn.textContent='Saving…';
    try{
      for(const d of changed){
        const full=S.projects.find(x=>x.id===d.id); if(!full)continue;
        const patched={...full,name:d.name.trim(),contractor:d.contractor.trim(),
          anticipatedCost:d.inHouse?full.anticipatedCost:(d.cost===''?null:Number(d.cost)),
          plannedStart:d.start,plannedEnd:d.end};
        await API.send('PATCH','/projects/'+d.id,projectPayload(patched,false));
      }
      scrim.remove();
      await afterWrite(changed.length+' phase'+(changed.length>1?'s':'')+' updated');
    }catch(e){ err.textContent='Save failed: '+e.message; saveBtn.disabled=false; saveBtn.textContent='Save changes'; }
  }},'Save changes');
  sheet.append(
    el('div',{class:'sh'}, el('h2',{style:'font-size:16px;flex:1'},'⧉ '+(sibs[0].phaseOf||'Phased program')+' — all phases'),
      el('button',{class:'btn ghost',onclick:()=>scrim.remove()},'Cancel'), saveBtn),
    el('div',{class:'sb'},
      el('div',{style:'overflow:auto;max-height:62vh'},t), err));
...
function openProject(id,preset){
  ...
  function drawPhases(){
    ...
    phasedStrip.append(el('span',{class:'ps-lab',style:'cursor:pointer',title:'Phased program · '+sibs.length+' phases · total '+fmt(total)+' — click to edit every phase (names, costs, dates) in one table',
      onclick:()=>{ const g=p.phaseGroup; close(); setTimeout(()=>openProgramEditor(g),60); }},'⧉ '+(p.phaseOf||'Program')));
    ...
    phasedStrip.append(dots, el('span',{style:'flex:1'}),
      el('button',{class:'btn ghost sm',title:'Edit every phase — names, costs, dates — in one table',
        onclick:()=>{ const g=p.phaseGroup; close(); setTimeout(()=>openProgramEditor(g),60); }},'⊞'),
  ...
  function drawPlan(){
    ...
    const seg=el('div',{class:'seg-ctl sm'},
      el('button',{class:!p.planKind?'on':'',onclick:()=>{p.planKind=null;drawPlan();}},'—'),
      el('button',{class:p.planKind==='completion'?'on':'',onclick:()=>{p.planKind='completion';drawPlan();}},'To completion'),
      el('button',{class:p.planKind==='recurring'?'on':'',onclick:()=>{p.planKind='recurring';drawPlan();}},'Recurring'));
    const lenderInp=el('input',{value:p.lenderFlag||'',placeholder:'e.g. Fannie',style:'width:110px',oninput:e=>p.lenderFlag=e.target.value});
    const fillAmt=el('input',{type:'number',placeholder:'annual $',style:'width:100px'});
    planBody.append(el('div',{style:'display:flex;gap:16px;align-items:end;flex-wrap:wrap;margin-bottom:10px'},
      el('div',{},lab('Kind'),seg),
      el('div',{},lab('Lender designation (blank = none)'),lenderInp),
      el('div',{},lab('Fill every year with'),
        el('span',{style:'display:inline-flex;gap:6px'},fillAmt,
          el('button',{class:'btn ghost sm',onclick:()=>{ const n=Number(fillAmt.value);
...
function viewProperty(){
  ...
  for(let q=currentQtr+1; q<=4; q++){
    const ov=distQtrs['retQ'+q], br=budgetByQ[q];
    const rate=(ov!=null && !isNaN(Number(ov))) ? Number(ov)
             : (br!=null && !isNaN(Number(br))) ? Number(br) : madePct;   // per-quarter override → budget → earnings
  ...
  // Base = Cash After Distribution (cushion Col V) — the report's authoritative post-distribution cash.
  // Fall back to current cash only when the latest cushion lacks Col V.
  const cashAfterDist = c.cashAfterDist!=null ? Number(c.cashAfterDist) : null;
  const projBase = cashAfterDist!=null ? cashAfterDist : cashToday;

  // Proj YYYY end Cash = Cash After Distribution + interest income + remaining Q net accretion − remaining SP spend
  const projEndCash = projBase
    + projInterest
    - projRemainingSpend
    + futureAccretion;
  const projEndTone = projEndCash<0?'bad':(projEndCash<projBase*0.25?'warn':'good');
  ...
  const allSettings=(overrides={})=>Object.assign({accretionPct:p.accretionPct??null,avgMonthlyInterest:avgInt,distributionQuarters:p.distributionQuarters||{}},overrides);
  ...
  function openAccretion(){
    const qs=[]; for(let q=currentQtr+1; q<=4; q++) qs.push(q);
    const budOr=q=>{ const br=budgetByQ[q]; return (br!=null && !isNaN(Number(br))) ? Number(br) : madePct; };   // report budget %, else earnings %
    let editedSent=effectiveSentPct;
    const edited={}; qs.forEach(q=>{ const ov=distQtrs['retQ'+q]; edited[q]=(ov!=null && !isNaN(Number(ov))) ? Number(ov) : budOr(q); });
    const scrim=el('div',{class:'scrim modal-center',onclick:e=>{if(e.target===scrim)scrim.remove();}});
    const sheet=el('div',{class:'sheet'}); const head=el('div',{class:'sh'}, el('h2',{style:'font-size:16px;flex:1'},'Remaining accretion · '+code), el('button',{class:'btn ghost',onclick:()=>scrim.remove()},'Close'));
    const b=el('div',{class:'sb'});
    b.append(el('p',{style:'margin-top:0;color:var(--ink-3);font-size:12.5px'},'Each remaining quarter accretes its return % net of the distribution rate, ÷ 4 × capital. Returns default to the cushion’s budgeted returns — edit any of them below. Never affects SP budget or spend.'));
    const body=el('div',{});
    const redraw=()=>{ body.innerHTML=''; let total=0; const rows=[];
      if(!qs.length) rows.push(detRow('No quarters remaining after the current one', 0));
      qs.forEach(q=>{ const a=((edited[q]-editedSent)/100/4)*capitalDollars; total+=a; rows.push(detRow('Q'+q+' — '+edited[q].toFixed(2)+'% return − '+editedSent.toFixed(2)+'% dist', a)); });
    ...
    const retInps={}; qs.forEach(q=>{ retInps[q]=el('input',{type:'number',step:'0.01',value:String(edited[q].toFixed(2)),style:'width:120px;padding:8px 10px;border:1px solid var(--line);border-radius:8px',oninput:e=>{edited[q]=parseFloat(e.target.value)||0;redraw();}}); });
    b.append(el('div',{class:'field'}, el('label',{},'Distribution rate % — cushion: '+sentPct.toFixed(2)+'%'), sentInp));
    qs.forEach(q=> b.append(el('div',{class:'field',style:'margin-top:8px'}, el('label',{},'Q'+q+' return % — budget: '+(budgetByQ[q]!=null?Number(budgetByQ[q]).toFixed(2)+'%':'—')), retInps[q])));
    b.append(body,
      el('div',{style:'display:flex;gap:8px;margin-top:12px'}, el('div',{style:'flex:1'}),
        el('button',{class:'btn ghost',onclick:()=>{ editedSent=sentPct; sentInp.value=sentPct.toFixed(2); qs.forEach(q=>{ edited[q]=budOr(q); retInps[q].value=edited[q].toFixed(2); }); redraw(); }},'Reset to budget'),
        el('button',{class:'btn accent',onclick:async()=>{
          ...
          qs.forEach(q=>{ if(Math.abs(edited[q]-budOr(q))<0.005){delete newQtrs['retQ'+q];}else{newQtrs['retQ'+q]=edited[q];} });
          scrim.remove();await saveSettings(allSettings({distributionQuarters:newQtrs}));
        }},'Save')
      )
    );
    sheet.append(head,b); scrim.append(sheet); document.body.append(scrim);
  ...
  // Proj end Cash tie-out: Cash After Distribution (cushion Col V) + interest + accretion − committed spend.
  function openProjEnd(){
    const scrim=el('div',{class:'scrim modal-center',onclick:e=>{if(e.target===scrim)scrim.remove();}});
    const sheet=el('div',{class:'sheet'}); const head=el('div',{class:'sh'}, el('h2',{style:'font-size:16px;flex:1'},'Proj '+fy+' end Cash · '+code), el('button',{class:'btn ghost',onclick:()=>scrim.remove()},'Close'));
    const b=el('div',{class:'sb'});
  ...
  function projRow(pr){
    ...
    const head=el('div',{class:'pr-head'},
      ...
      el('strong',{class:'pr-name'}, lenderSym(pr), pr.name), projSym(pr), phaseChip(pr), qtyChip(pr),
...
/* =========================================================
   LONG-RANGE PLAN — loan-term horizon (TRMO Fannie tracker, 028)
   Horizon = this year → the property's loan-due year (Settings override),
   plus a Post-Refi bucket for work deferred until cash replenishes after
   refinance. Planning overlay only — projections/GL tie-out never read it.
========================================================= */
function viewPlan(){
  if(PLANV.prop&&!PROP(PLANV.prop))PLANV.prop='';
  return PLANV.prop?planDetail(PLANV.prop):planSummary();
}
function planExcelBtn(code){
  return el('a',{class:'btn ghost sm',href:'/api/export/plan.xlsx'+(code?'?property='+encodeURIComponent(code):''),
    title:'Download the plan workbook (TRMO tracker layout, with a raw-data sheet)',
    onclick:()=>toast('Plan workbook downloading…')},'⬇ Excel');
}
async function savePlanCells(p,msg){
  try{ await saveProjectSilent(p); await refreshState(); }
  catch(e){ toast('Save failed: '+e.message); try{ await refreshState(); }catch(_e){} }
  render(); if(msg)toast(msg);
}
...
function planSummary(){
  const bar=topbar('Money','Long-Range Plan', planExcelBtn(''));
  const body=el('div');
  const years=new Set(); S.properties.forEach(pr=>planYearColsFor(pr.code).forEach(y=>years.add(y)));
  const cols=[...years].sort();
  const panel=el('div',{class:'panel'});
  panel.append(el('div',{class:'ph'},el('h3',{},'Portfolio plan by year'),el('div',{class:'sp'}),
    el('span',{class:'chip'},'horizon = loan term remaining')));
  const t=el('table',{class:'tbl'});
  ...
  propsByRegion().forEach(pr=>{
    ...
    const row=tr(
      ...
      td(S.cash[code]&&S.cash[code].loanDue?S.cash[code].loanDue:'—'),
      td(String(end)));
    cols.forEach(y=>{
      const inH=+y<=end;
    ...
    row.onclick=()=>{PLANV.prop=code;render();};
    tb.append(row);
  });
  const trow=tr(td(el('strong',{},'TOTAL')),td(''),td(''));
  cols.forEach(y=>trow.append(td(el('strong',{},fmt(totals[y]||0)),'r')));
  trow.append(td(el('strong',{},fmt(totals[PLAN_POST]||0)),'r'), td(el('strong',{},fmt(gTot)),'r'), td(el('strong',{},fmt(gUn)),'r'));
  tb.append(trow);
  t.append(tb);
  panel.append(el('div',{style:'overflow:auto'},t));
  body.append(panel);
...
function planDetail(code){
  ...
  const bar=topbar('Money · Long-Range Plan', code+' — '+pr.name,
    ...
    planExcelBtn(code),
    el('button',{class:'btn accent sm',onclick:()=>openProject(null,{property:code})},'＋ New plan item'));
  const body=el('div');
  const stat=(l,v)=>el('div',{},el('div',{style:'font-size:11px;color:var(--ink-3);text-transform:uppercase;letter-spacing:.05em'},l),el('div',{style:'font-weight:600'},v));
  ...
  const t=el('table',{class:'tbl'});
  const hd=tr(th('Item'),th(''));
  keys.forEach(k=>hd.append(th(planYearLabel(k),'r')));
  ...
  const programRow=(g,members)=>{
    ...
    const row=tr(
      td(el('a',{href:'javascript:void 0',style:'font-weight:700;color:var(--ink)',title:'Edit every phase (names, costs, dates) in one table',
        onclick:()=>openProgramEditor(g)},'⧉ '+(members[0].phaseOf||'Phased program'))),
  ...
  if(!notIn.length)ub.append(el('span',{style:'color:var(--ink-3);font-size:12.5px'},'Every open project at '+code+' is in the plan — this year’s items flow in automatically.'));
    ...
    notIn.forEach(p=>{
      ...
      utb.append(tr(
        ...
        td(p.category||''),
        tdn(est||null,true),
        td(el('button',{class:'btn ghost sm',title:'Put on the plan — seeds this year with the anticipated cost (adjust after)',onclick:()=>{
          if(Number(p.anticipatedCost)>0){ p.planYears={[String(nowYear)]:Number(p.anticipatedCost)}; savePlanCells(p,'Scheduled into '+nowYear+' — adjust the years in the grid'); }
          else openProject(p.id);
        }},'Schedule ↴'))));
    });
    ut.append(utb);
    ub.append(el('div',{style:'overflow:auto'},ut));
  }
  up.append(ub);
  body.append(up);
  return {bar,body};
...
function openPropertyEditor(code){
  ...
  b.append(
    ...
    field('Plan horizon end year (blank = loan-due year from the cushion)','planEndYear',pr?pr.planEndYear:'',{type:'number',placeholder:'e.g. 2031'}));
  if(pr){
  ...
  const save=async()=>{
    const payload={ code:pr?pr.code:f.code.value, name:f.name.value, region:f.region.value, manager:f.manager.value,
      ...
      planEndYear:f.planEndYear.value };
    try{
