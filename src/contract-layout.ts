...
/* ---------- Exhibit E: the blank change-order form ----------
   A bordered grid, not prose — the only exhibit that is a fillable form. It is
   deliberately blank: it gets printed and completed by hand when a change
   arises (the FILLED version is generated by change-order.ts). ONE drawing of
   the form, shared by BOTH contract templates (SP contract.ts and multi-entity
   contract-multi.ts), so the two can never drift apart. */
export function changeOrderFormPage(doc: PDFDocument, roman: PDFFont, bold: PDFFont): void {
