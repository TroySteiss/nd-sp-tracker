...
export function rowToProject(r: any, bids: Bid[] = [], notes: ProgressNote[] = [], props?: PropLookup): Project {
  ...
  return {
    ...
    // Long-range plan (028): per-year $ + "post" bucket, kind, lender flag.
    planYears: r.plan_years ?? null,
    planKind: r.plan_kind === 'recurring' ? 'recurring' : r.plan_kind === 'completion' ? 'completion' : null,
    lenderFlag: r.lender_flag ?? '',
    steps: r.steps || {},
    notes: r.notes ?? '',
    onHold: !!r.on_hold,
    pinned: !!r.pinned,
