...
import {
  ...
  PLAN_POST, onPlan, planFor, planTotal, shareFor, involvesProp, isSplit, allocsOf,
...
/** A property's plan rows: scheduled (on the plan) + unscheduled candidates. */
function rowsFor(state: AppState, code: string): { scheduled: Project[]; unscheduled: Project[] } {
  ...
  const unscheduled = projs.filter((p) => !onPlan(p) && phase(p) !== 'done')
...
export function buildPlanWorkbook(state: AppState, codes: string[], now: Date): Buffer {
  ...
  const allProjs = [...new Map(codes.flatMap((c) => { const r = perProp.get(c)!; return [...r.scheduled, ...r.unscheduled]; }).map((p) => [p.id, p])).values()];
  ...
  const RAW_META = ['Lead property', 'Split shares', 'Project ID', 'Project', 'Category', 'Lender flag', 'Kind', 'In-house / Contractor', 'Contractor', 'Next steps', 'Status', 'Anticipated cost', 'Actual cost'];
  const RAW_Y0 = RAW_META.length;                       // first year column
  setCell(raw, 0, 0, txt('RAW DATA — full project amounts (not share-weighted). Property sheets multiply these by each property’s share, so a split project’s slices are reviewable.'));
  ...
  allProjs.forEach((p, i) => {
    ...
    setCell(raw, r, 7, txt(p.inHouse ? 'In-house' : 'Contractor'));
    setCell(raw, r, 8, txt(p.contractor || ''));
    setCell(raw, r, 9, txt(p.actionItem || ''));
    setCell(raw, r, 10, txt(statusOf(p)));
    if (p.anticipatedCost != null) setCell(raw, r, 11, num(Number(p.anticipatedCost), MONEY));
    if (p.actualCost != null) setCell(raw, r, 12, num(Number(p.actualCost), MONEY));
    unionKeys.forEach((k, i2) => { const v = planFor(p, k); if (v > 0) setCell(raw, r, RAW_Y0 + i2, num(v, MONEY)); });
  });
  raw['!cols'] = [{ wch: 12 }, { wch: 22 }, { wch: 10 }, { wch: 46 }, { wch: 20 }, { wch: 10 }, { wch: 13 }, { wch: 18 }, { wch: 18 }, { wch: 28 }, { wch: 18 }, { wch: 14 }, { wch: 12 }, ...unionKeys.map(() => ({ wch: 12 }))];
  ...
  for (const code of codes) {
    ...
    const { scheduled, unscheduled } = perProp.get(code)!;
    const endYear = endOf(code);
    const yearKeys = [...planYearCols(scheduled, endYear, nowYear), PLAN_POST];
    ...
    scheduled.forEach((p, i) => {
      ...
      setCell(ws, r, PC.kind, txt(p.planKind === 'recurring' ? 'Recurring' : 'Completion'));
      ...
      setCell(ws, r, cTotal, fml(`SUM(${y0Addr}:${yNAddr})`, round2(planTotal(p) * share), MONEY));
      if (p.anticipatedCost != null) setCell(ws, r, cEst, fml(`'Raw Data'!${A(rr, 11)}*${shareAddr}`, round2(Number(p.anticipatedCost) * share), MONEY));
      if (p.actualCost != null) setCell(ws, r, cAct, fml(`'Raw Data'!${A(rr, 12)}*${shareAddr}`, round2(Number(p.actualCost) * share), MONEY));
    ...
    const totR = first + scheduled.length;
    setCell(ws, totR, PC.desc, txt('TOTAL'));
    const byKey = new Map<string, string>();
    const sumCol = (c: number, cached: number) => scheduled.length
      ? fml(`SUM(${A(first, c)}:${A(totR - 1, c)})`, round2(cached), MONEY)
      : num(0, MONEY);
    yearKeys.forEach((k, i2) => {
      const cached = scheduled.reduce((a, p) => a + planFor(p, k) * shareFor(p, code), 0);
      setCell(ws, totR, PC.y0 + i2, sumCol(PC.y0 + i2, cached));
      byKey.set(k, A(totR, PC.y0 + i2));
    });
    setCell(ws, totR, cTotal, sumCol(cTotal, scheduled.reduce((a, p) => a + planTotal(p) * shareFor(p, code), 0)));
    setCell(ws, totR, cEst, sumCol(cEst, scheduled.reduce((a, p) => a + (Number(p.anticipatedCost) || 0) * shareFor(p, code), 0)));
    setCell(ws, totR, cAct, sumCol(cAct, scheduled.reduce((a, p) => a + (Number(p.actualCost) || 0) * shareFor(p, code), 0)));

    // Unscheduled / future items (the TRMO "Future SP Items" sheet, inline).
    let unschedEstAddr: string | null = null;
    if (unscheduled.length) {
      const uHead = totR + 2;
      setCell(ws, uHead, PC.desc, txt('UNSCHEDULED / FUTURE ITEMS — no year assigned yet'));
      unscheduled.forEach((p, i) => {
        const r = uHead + 1 + i;
        const rr = rawRowOf.get(p.id)!;
        const share = shareFor(p, code);
        const shareAddr = `$${XLSX.utils.encode_col(PC.share)}${r + 1}`;
      ...
      const uTot = uHead + 1 + unscheduled.length;
      setCell(ws, uTot, PC.desc, txt('UNSCHEDULED TOTAL (est.)'));
      setCell(ws, uTot, cEst, fml(`SUM(${A(uHead + 1, cEst)}:${A(uTot - 1, cEst)})`,
        round2(unscheduled.reduce((a, p) => a + (Number(p.anticipatedCost) || 0) * shareFor(p, code), 0)), MONEY));
  ...
  if (codes.length > 1) {
    ...
    setCell(ws, HEAD, Y0 + unionKeys.length + 1, txt('Unscheduled est.'));
    codes.forEach((code, i) => {
      ...
      setCell(ws, r, Y0 + unionKeys.length, fml(`'${code}'!${t.planTotal}`, round2(scheduled.reduce((a, p) => a + planTotal(p) * shareFor(p, code), 0)), MONEY));
      if (t.unschedEst) setCell(ws, r, Y0 + unionKeys.length + 1, fml(`'${code}'!${t.unschedEst}`, round2(unscheduled.reduce((a, p) => a + (Number(p.anticipatedCost) || 0) * shareFor(p, code), 0)), MONEY));
    });
