...
api.post('/projects/:id/countersign', requireAdmin, async (req, res) => {
  ...
  const steps: Record<string, boolean> = { ...(proj.steps || {}), signed: true };
  await query('update projects set executed_contract_file_key=$1, executed_contract_file_name=$2, steps=$3, updated_at=now() where id=$4',
    [key, fileName, JSON.stringify(steps), proj.id]);
  logChange(req, { action: 'contract.countersign', entityType: 'project', entityId: proj.id, property: proj.property_code, summary: `Contract countersigned in-app — "${fileName}" attached as the executed contract on "${proj.name}"` });
  res.json({ fileKey: key, fileName, steps });
});
