  ...
  for (const p of state.projects || []) {
    await client.query(
      `insert into projects(id,property_code,category,name,description,plan,action_item,contractor,
         ...
         plan_years,plan_kind,lender_flag,phase_group,phase_seq,phase_of,quantity,quantity_unit)
      ...
      [
        ...
        nnull((p as any).quantity), (p as any).quantityUnit || null,
      ]
    );
    let slot = 0;
  ...
  for (const code of Object.keys(state.cash || {})) {
    ...
    await client.query(
      `insert into cash_snapshots(property_code,as_of_date,cash,adj_cash,sp_budget,sp_spent,sp_remaining,
         ...
         budget_ret_q1,budget_ret_q2,budget_ret_q3,budget_ret_q4)
      ...
      [code, dnull(c.asOfDate), nnull(c.cash), nnull(c.adjCash), nnull(c.spBudget), nnull(c.spSpent), nnull(c.spRemaining),
       ...
       nnull((c as any).budgetRetQ1), nnull((c as any).budgetRetQ2), nnull((c as any).budgetRetQ3), nnull((c as any).budgetRetQ4)]
    );
  }
